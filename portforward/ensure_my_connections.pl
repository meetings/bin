...
my ( $dir ) = Cwd::abs_path($0) =~ /(.*portforward)/;
my $default_dir = $dir;
...
$front ||= `/usr/bin/ssh-keygen -R 10.0.0.1 2>/dev/null; /bin/chmod 700 $rsa_file; /usr/bin/ssh -o "StrictHostKeyChecking no" -o "CheckHostIP no" -i $rsa_file portforward\@10.0.0.1 /bin/hostname 2>/dev/null`;
chomp $front;
